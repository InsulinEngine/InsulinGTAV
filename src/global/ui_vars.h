...
// color_rgba's constructors are constexpr so the g_* colour globals in
// ui_vars.cpp are constant-initialised (statically). Without that they would be
// dynamic-init, which GoldHEN never runs (no .init_array), leaving every colour
// zeroed (black/transparent).
struct color_rgba {
    int r;
    int g;
    int b;
    ...
    constexpr color_rgba() : r(0), g(0), b(0), a(0) {}
    constexpr color_rgba(int red, int green, int blue, int alpha = 255)
        : r(red), g(green), b(blue), a(alpha) {}
    constexpr color_rgba(uint32_t hex)
        : r(hex >> 24), g((hex >> 16) & 0xFF), b((hex >> 8) & 0xFF), a(hex & 0xFF) {}
    ...
    uint32_t to_argb() {
        uint8_t val[4];
        val[0] = a; val[1] = r; val[2] = g; val[3] = b;
        return *(uint32_t*)&val;
    }
};
...
struct radio_context {
    stl::unordered_map<int, bool> m_toggles;
    int m_selected = 0;
    int m_count = 0;
    stl::pair<const char*, const char*> m_sprite = { "", "" };

    radio_context() {}
    radio_context(const char* dict, const char* texture) {
        m_sprite = stl::make_pair(dict, texture);
        m_selected = 0;
        m_count = 0;
    }
    ...
    void reset() {
        m_count = 0;
        for (auto& it : m_toggles) it.second = false;
    }
};
...
struct menu_texture {
    ...
    stl::string m_texture;
    localization m_submenu_name;
    color_rgba* m_color = nullptr;

    menu_texture() {}
    menu_texture(color_rgba* color) { m_color = color; }
};
...
namespace global::ui {
    // One-time binding of the string/pointer-bearing texture globals. These
    // cannot be constant-initialised (they hold stl::string / bind a colour
    // pointer), so they are left zero-initialised (a valid empty texture) and
    // fixed up here, called from menu::build() on the game thread.
    void init();

    extern float g_option_scale;
    extern float g_option_height;

    extern stl::string g_render_queue[100];
    extern stl::string g_rendering_tooltip;
