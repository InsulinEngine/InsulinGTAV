...
namespace global::ui {
    ...
    float g_option_height = 0.315f;   // was g_option_scale * 10.f (constant-folded)

    stl::string g_render_queue[100];
    stl::string g_rendering_tooltip;
    line_2d* m_line_2d;
    ...
    // constexpr color_rgba ctor => these are constant-initialised (static),
    // so GoldHEN skipping .init_array does not zero them.
    ...
    menu_texture m_header;
    menu_texture m_background;
    menu_texture m_scroller;
    menu_texture m_footer;
    ...
    void init() {
        m_header.m_color = &g_main_header;
        m_background.m_color = &g_background;
        m_scroller.m_color = &g_scroller;
        m_footer.m_color = &g_footer;

        // Restore the default sprite bindings (default member initializer is
        // skipped when .init_array does not run).
        m_header.m_context = radio_context("commonmenu", "shop_art_icon");
        m_background.m_context = radio_context("commonmenu", "shop_art_icon");
        m_scroller.m_context = radio_context("commonmenu", "shop_art_icon");
        m_footer.m_context = radio_context("commonmenu", "shop_art_icon");
    }
}
