...
namespace menu::notify {
    void notify::update() {
        if (global::ui::g_stop_rendering) {
            for (notify_context* context = m_context.begin(); context != m_context.end(); context++) {
                context->m_start_time = platform::now_ms();
            }
            return;
        }
        ...
        for (notify_context* context = m_context.begin(); context != m_context.end(); context++) {
            ...
            if (context->m_has_init) continue;
            ...
            if (context->m_text.size() == 1) {
                ...
                for (size_t i = 0; i < context->m_text.size(); i++) {
                    float width = menu::renderer::calculate_string_width(context->m_text[i], 0, text_height) + (x_offset * 2.f);
                    if (context->m_max_width < width) context->m_max_width = width;
                }
        ...
        float y = 0.01f;
        // reverse iteration (Ozark uses rbegin/rend)
        for (int idx = (int)m_context.size() - 1; idx >= 0; idx--) {
            notify_context* context = &m_context[idx];
            float x_offset = 0.006f;
            ...
            if (end_time < platform::now_ms() || end_time - platform::now_ms() < 1000) {
                death = true;
                context->m_y = math::lerp(context->m_y, 1.2f, 5.f * global::ui::g_delta);

                if (context->m_alpha_start == 0) context->m_alpha_start = platform::now_ms();
                context->m_alpha = 255 - (((platform::now_ms() - context->m_alpha_start) * 255) / 1000);
            } else {
                if (context->m_title_width > context->m_max_width) wrap = context->m_title_width + x_offset;
            }

            if (context->m_title_width == 0.f) {
                context->m_title_width = menu::renderer::calculate_string_width(stl::string("~bold~") + context->m_title, global::ui::g_notify_title_font, scaled_title_height) + (0.004f * 2.f);
            }
            ...
            if (context->m_text.size() == 1) {
                ...
                stl::string full_line = "";
                context->m_max_width = menu::renderer::calculate_string_width(context->m_text[0], global::ui::g_notify_body_font, scaled_body_height) + (x_offset * 2.f);

                for (size_t i = 0; i < context->m_text.size(); i++) {
                    float width = menu::renderer::calculate_string_width(context->m_text[i], global::ui::g_notify_body_font, scaled_body_height) + (x_offset * 2.f);
                    if (context->m_max_width < width) context->m_max_width = width;

                    full_line += context->m_text[i];
                    if (i != (context->m_text.size() - 1)) full_line += '\n';
                }
            ...
            if (!death) context->m_y = math::lerp(context->m_y, y, 5.f * global::ui::g_delta);

            if (context->m_y + height > 0.7f) context->m_time_limit = 0;
            ...
            if (context->m_y <= 1.f) {
                ...
                stl::pair<stl::string, stl::string> texture = menu::renderer::get_texture(global::ui::m_notify_background);
                ...
                menu::renderer::draw_text(stl::string("~bold~") + context->m_title, { context->x + x_offset, context->m_y + 0.003f + 0.001f }, scaled_title_height, global::ui::g_notify_title_font, { 255, 255, 255, context->m_alpha }, JUSTIFY_LEFT);
                y += height + 0.004f;
            }
        }

        for (notify_context* context = m_context.begin(); context != m_context.end();) {
            if (context->m_start_time + context->m_time_limit < platform::now_ms()) {
    ...
    void notify::stacked(stl::string title, stl::string text, color_rgba color, uint32_t timeout) {
    ...
    void notify::stacked_lines(stl::string title, stl::vector<stl::string> text, color_rgba color) {
        notify_context context;
        for (stl::string& str : text) context.m_text.push_back(str);
