...
public:
    scroll_option(scroll_option_type type, stl::string name)
    ...
    scroll_option& add_toggle(bool& t) { m_toggle = &t; m_toggle_cache = t; return *this; }
    scroll_option& add_click(stl::function<void()> function) { m_on_click = function; return *this; }
    scroll_option& add_requirement(stl::function<bool()> function) { m_requirement = function; return *this; }
    scroll_option& add_update(stl::function<void(scroll_option*, int)> function) { m_on_update = function; return *this; }
    scroll_option& add_hover(stl::function<void()> function) { m_on_hover = function; return *this; }
    scroll_option& add_tooltip(stl::string tooltip) { m_tooltip.set(tooltip.c_str()); return *this; }
    scroll_option& add_translate() { m_name.set_translate(true); m_tooltip.set_translate(true); return *this; }
    scroll_option& add_hotkey() { m_has_hotkey = true; return *this; }
    scroll_option& set_scroll_speed(uint32_t speed) { m_scroll_speed = speed; return *this; }
    scroll_option& loop() { m_loop = true; return *this; }
    scroll_option& add_offset(float offset) { m_offset = offset; return *this; }
    scroll_option& add_savable(stl::stack<stl::string> /*menu_stack*/) { return *this; }
    ...
    void render(int position) {
        ...
        snprintf(name_buffer, sizeof(name_buffer), "%s ~m~[%i/%i]", m_scroll[*m_number].m_name.get().c_str(), (*m_number) + 1, m_max + 1);
    ...
    void render_selected(int position, stl::stack<stl::string> submenu_name_stack) {
        ...
        if (m_requirement() && menu::input::is_option_pressed()) {
            if (m_type == TOGGLE && m_toggle) *m_toggle = !*m_toggle;
            m_on_click();
        }
...
private:
    ...
    stl::function<void(scroll_option*, int)> m_on_update = [](scroll_option*, int) {};
